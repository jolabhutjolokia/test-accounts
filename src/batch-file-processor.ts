...
import { parseBalancesFile, ParsedBalance } from './input-parsers/csv/balance-csv-parser';
import { ParsedTransaction, parseTransactionsFile } from './input-parsers/csv/transactions-csv-parser';
import { Money } from './models/money';
import { FailureDetails } from './models/failures';

function createInitialBalances(balances: ParsedBalance[], service: BankAccountService) {
...
function processAllTransactions(transactions: ParsedTransaction[], service: BankAccountService) {
...
export const createBatchFileProcessor = (service: BankAccountService) => (args: {
  balanceFile: string;
  transactionsFile: string;
}) => {
  const balances = parseBalancesFile(args.balanceFile);
  const transactions = parseTransactionsFile(args.transactionsFile);
  const balanceCreationFailures = createInitialBalances(balances.successes, service);
  const transactionProcessingFailures = processAllTransactions(transactions.successes, service);
